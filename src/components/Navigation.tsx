...
const COURSE_CATEGORIES = [
  ...
  { icon: <Layers className="w-5 h-5 text-teal-500" />, name: 'Specialized Industry Tracks', slug: 'specialized-industry-tracks', count: 0 },
  ...
  { icon: <Calendar className="w-5 h-5 text-orange-400" />, name: 'Hospitality, Tourism and Events', slug: 'hospitality-tourism-and-events', count: 0 },
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3">
        <div className="flex justify-between items-center">
          ...
          <nav className="hidden lg:flex space-x-1" ref={dropdownRef}>
            ...
            {/* Discover Career Dropdown */}
            <div className="relative">
              <button
                onClick={() => toggleDropdown('career')}
                className={`px-3 py-2 text-base font-medium text-white ${deepBlueHover} rounded transition-all duration-200 flex items-center border border-transparent hover:border-yellow-400/30 hover:shadow-lg hover:shadow-yellow-400/20 ${activeDropdown === 'career' ? 'bg-gradient-to-r from-[#11235a] via-[#2a3a7b] to-[#11235a]' : ''}`}
              >
                <Compass className="w-4 h-4 mr-1" />
                DISCOVER CAREER
                <ChevronDown className={`ml-1 h-4 w-4 transition-transform ${activeDropdown === 'career' ? 'rotate-180' : ''}`} />
              </button>
              {activeDropdown === 'career' && (
                <div className="absolute left-0 mt-2 w-96 bg-white rounded-lg shadow-xl py-2 z-50 border border-gray-200 backdrop-blur-sm bg-white/95">
                  <div className="px-4 py-2 border-b border-gray-100 text-xs text-gray-500 uppercase tracking-wider font-semibold">EXPLORE CAREER CATEGORIES</div>
                  {CAREER_CATEGORIES.map((category) => {
                    ...
                    const routeMap: { [key: string]: string } = {
                      ...
                      'leadership-and-management': '/career/leadership-and-management',
                      'mentorship-career-readiness': '/career/mentorship-career-readiness',
                      'specialised-industry-tracks': '/career/specialised-industry-tracks',
                      'real-estate-and-estate-management': '/career/real-estate-and-estate-management',
                      'public-safety-and-emergency-services': '/career/public-safety-and-emergency-services',
                      'social-care-and-community-support': '/career/social-care-and-community-support',
                      'health-science': '/career/health-science',
                      'finance': '/career/finance',
                      'information-technology': '/career/information-technology',
                      'education-training': '/career/education-training',
                      'business-management-administration': '/career/business-management-administration',
                      'marketing-sales-service': '/career/marketing-sales-service',
                      'agriculture-food-natural-resources': '/career/agriculture-food-natural-resources',
                      'hospitality-tourism': '/career/hospitality-tourism'
                    };
                    
                    const route = routeMap[category.slug] || `/career/${category.slug}`;
